...
def init_db():
    ...
    cursor.execute("SELECT id FROM users WHERE username = 'overseer'")
    if not cursor.fetchone():
        admin_hash = bcrypt.hashpw(b"Overseer2077!", bcrypt.gensalt(rounds=12))
        cursor.execute(
            "INSERT INTO users (username, password_hash, role) VALUES (%s, %s, %s)",
            ("overseer", admin_hash.decode("utf-8"), "admin")
        )
        app.logger.info("👤 Usuario 'overseer' creado")

    # Insertar usuario normal de demo si no existe
    cursor.execute("SELECT id FROM users WHERE username = 'dweller'")
    if not cursor.fetchone():
        user_hash = bcrypt.hashpw(b"Dweller2077!", bcrypt.gensalt(rounds=12))
        cursor.execute(
            "INSERT INTO users (username, password_hash, role) VALUES (%s, %s, %s)",
            ("dweller", user_hash.decode("utf-8"), "user")
        )
        app.logger.info("👤 Usuario 'dweller' creado")

    # Insertar proyectos de demo si la tabla está vacía
    cursor.execute("SELECT COUNT(*) FROM projects")
    if cursor.fetchone()[0] == 0:
        projects = [
            ("Proyecto Overseer Alpha",  "Migración de infraestructura a la nube", "activo",      "admin"),
            ("Protocolo Vault-Tec Beta",   "Rediseño del portal de clientes",         "en revisión", "usuario"),
            ("Directiva Seguridad Gamma",  "Automatización de pruebas QA",            "completado",  "usuario"),
            ("Operacion Delta",  "Integración con API de pagos",            "activo",      "admin"),
            ("Iniciativa OWASP Epsilon","Actualización de seguridad OWASP",        "en revisión", "admin"),
        ]
        cursor.executemany(
...
def check_rate_limit(ip: str) -> bool:
    """
    Máximo 10 intentos de login por IP en ventana de 15 minutos.
